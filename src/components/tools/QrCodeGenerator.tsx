  ...
  return (
    <div className="grid gap-6 sm:grid-cols-[1fr_auto]">
      <div className="space-y-4">
        <div>
          ...
          <textarea
            ...
            className="h-28 w-full resize-y rounded-xl border border-gray-300 bg-gray-50 p-3 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100"
          />
        ...
        <div>
          <div className="mb-1 flex justify-between text-sm">
            ...
            <span className="font-mono text-gray-700">{size} × {size} px</span>
          </div>
          <input
            type="range"
            min={128}
        ...
        <div>
          <label className="mb-1.5 block text-sm font-medium text-gray-600">纠错级别</label>
            ...
            {ERROR_LEVELS.map((l) => (
              <button
                ...
                className={
                  level === l.value
                    ? btnPrimary
                    : 'rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm text-gray-700 transition hover:bg-gray-50'
                }
              >
                {l.label}
              </button>
      ...
      <div className="flex flex-col items-center gap-2">
        ...
        <p className="text-xs text-gray-400">{text.trim() ? '手机扫码即可查看内容' : '输入内容后自动生成'}</p>
      </div>
    </div>
  );
}
